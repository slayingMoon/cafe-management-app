...
export class ManageOrderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getCategories(); //we are loading this method, each time the page loads
  ...
  getProductDetails(value: any) {
    this.productService.getById(value.id).subscribe((response: any) => {
      ...
      this.manageOrderForm.controls['total'].setValue(this.price * 1); //may cause a problem
    }, (error: any) => {
      this.ngxService.stop();
  ...
  validateProductAdd() {
    if (this.manageOrderForm.controls['total'].value === 0 || this.manageOrderForm.controls['total'].value === null
      ...
      return true; //because it checks whether or not to disable the button (true-> disable; false -> enable)
    } else {
      return false;
    }
  }
